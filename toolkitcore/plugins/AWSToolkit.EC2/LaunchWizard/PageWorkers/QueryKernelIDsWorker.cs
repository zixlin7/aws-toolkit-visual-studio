...
namespace Amazon.AWSToolkit.EC2.LaunchWizard.PageWorkers
{
    ...
    /// </summary>
    internal class QueryKernelIDsWorker
    {
        ...
        public QueryKernelIDsWorker(IAmazonEC2 ec2Client,
                                    string amiID,
                                    ILog logger,
        ...
        void Worker(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                DescribeImageAttributeRequest request
                        = new DescribeImageAttributeRequest() { ImageId = workerData.AmiID, Attribute = "kernel" };

                DescribeImageAttributeResponse response = workerData.EC2Client.DescribeImageAttribute(request);
                kernelIDs.Add(response.ImageAttribute.KernelId);
            }
            catch (Exception exc)
            {
